...
- **Z.ai 공식 로고**: 공식 로고 사용
- **앱 자동 업데이트(Sparkle)**: `GLMBar.app`에서 앱 내부 업데이트 확인 및 자동 업데이트 설정을 지원합니다.
...
터미널 바이너리는 수동 업데이트 대상입니다. 자동 업데이트는 `GLMBar.app`에만 적용됩니다.
...
`GLMBar.app`은 Sparkle 기반 앱 내부 업데이트를 지원합니다.

- 수동 확인: 앱 팝업 열기 -> `Check for Updates...`
- 자동 업데이트 설정: 앱 팝업 -> `Settings...` -> `Automatic Updates` 토글
- 피드 URL: `https://uwseoul.github.io/glm-bar/appcast.xml`

`glm-bar` CLI 바이너리는 Sparkle 대상이 아니므로 Releases에서 수동 업데이트해야 합니다.
